	...
	var Board = function(){
		...
		this.getGroup = function(bubble,found,differentColor){
			if(bubble.getType() == 4){
				var bombGroup = {};
				bombGroup.list = [];
				bombGroup.list.push(bubble);
				var bombRow = bubble.getRow();
				var bombCol = bubble.getCol();
				var toExplode = that.getBubblesAround(bombRow, bombCol);
				for(var i=0;i<toExplode.length;i++){
					bombGroup.list.push(toExplode[i]);
				};
				return bombGroup;
			}else{
